...
if($menuOpen.Count){
  ...
  Write-Output "  ...or, when the pages are in the VIDEO MANAGER's menu domain (no VTS_xx_0.VOB on"
  Write-Output "  the disc, PGCs listed under menu0p*), the same tool reads that domain too:"
  Write-Output "  dvd-still-cells.py --vmgm <VIDEO_TS> <out> <pgc,pgc,...>      (takes NO vts number)"
