...
def transcribe(model, src, track, offsets, dur):
    ...
    lang = max(set(l for l, _ in langs), key=[l for l, _ in langs].count) if langs else None
    prob = max([p for l, p in langs if l == lang], default=0.0) if langs else 0.0
    return lang, prob, texts
...
def main():
    ...
    for s in streams:
        lang, prob, texts = transcribe(model, src, s['a'], offsets, a.dur)
        s['spokenLang'] = lang
        s['langProb'] = prob
        s['samples'] = texts
        s['tagMismatch'] = bool(lang and s['langTag'] and lang[:2] != s['langTag'][:2]
                                and not (lang == 'en' and s['langTag'] == 'eng'))
        print(f"  a:{s['a']} {s['codec']:9} {str(s['channels']):>2}ch tag={s['langTag']}"
              f" spoken={lang}({prob})" + ('  <<< TAG MISMATCH' if s['tagMismatch'] else ''))
    ...
    for s in streams:
        text = ' '.join(s['samples'])
        sim = difflib.SequenceMatcher(None, ptext.lower(), text.lower()).ratio() if ptext else 0.0
        s['similarityToPrimary'] = round(sim, 2)
        if s['redundantWith'] is not None:
            s['role'] = 'redundant'
        elif s is primary:
            s['role'] = 'primary'
        elif s['spokenLang'] and s['spokenLang'] != (primary['spokenLang'] if primary else 'en'):
            s['role'] = 'dub'
    ...
    keep = [s['a'] for s in streams if s['role'] in ('primary', 'commentary', 'audioDescription')]
    proposal = {
        'audioTracks': keep,
    ...
    for s in streams:
        ...
        if s['role'] == 'unknown':
            warnings.append(f"a:{s['a']} unclassified - decide by hand")
